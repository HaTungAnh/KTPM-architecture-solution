...
const UrlShortener = () => {
    ...
    const shortenLink = async (e) => {
        ...
        try {
            const res = await fetch(apiConfig.api + `/api/create?url=${url}`, {
                method: "POST",
                headers: {
            ...
            if (json && json.short) {
                ...
                const fetchedLink = apiConfig.api + `/${json.short}`;
                console.log(fetchedLink);
                setLinks([...links, {
                    full_short_link: fetchedLink,
                    original_link: url,
                }]);
            } else {
                setError(true);
    ...
    return (
        <div className="mx-5 lg:mx-20 xl:mx-36 -mt-24 md:-mt-20" style={{marginBottom: '400px'}}>
            <form onSubmit={shortenLink}>
              ...
              Please add valid a url
                    ...
                    <button
                        className="w-full md:w-[180px] text-white font-semibold py-3 md:py-4 px-4 md:px-8 bg-primary-cyan hover:bg-[#63dddd] rounded-md transition-colors duration-200">
                        Shorten it!
                    </button>
                </div>
            </form>
